...
export default function SplashScreen({ navigation, route }: any) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // 🟢 Logic: Route based on first launch status
      if (isFirstLaunch) {
        navigation.replace("Onboarding");
      } else {
        navigation.replace("Login");
      }
    }, 4000); // Note: I changed 10000 (10s) to 4000 (4s). 10 seconds is usually too long for a splash screen!
